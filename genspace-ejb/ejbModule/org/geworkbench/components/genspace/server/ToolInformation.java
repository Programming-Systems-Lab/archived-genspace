 ...
 */
@Stateless
public class ToolInformation extends AbstractFacade<Tool> implements ToolInformationRemote,ToolInformationLocal {
	...
	public List<Tool> getToolsByPopularity(User requestor) {
	...
	public List<Workflow> getWorkflowsByPopularity(User requestor) {
	...
	@SuppressWarnings("unchecked")
	public List<Tool> getMostPopularWFHeads(User requestor) {
		Query q = getEntityManager().createQuery("select object(c) from Tool as c order by c.wfCountHead");
        List<Tool> r = q.getResultList();
        return r;
	}

	public Tool getMostPopularNextTool(User requestor,Tool tool) {
	...
	public Tool getMostPopularPreviousTool(User requestor,Tool tool) {
		Query q = getEntityManager().createNativeQuery("select t.* from TOOL t " +
				"inner join WORKFLOWTOOL wt on wt.tool_id=t.id " +
				"inner join WORKFLOWTOOL wt2 on wt2.workflow_id=wt.workflow_id and wt2.cardinality = wt.cardinality+1 " +
				"inner join TOOL t2 on t2.id=wt2.tool_id " +
	...
	public Transaction logUsageEvent(User myUser, AnalysisEvent e) {
		if(
			(e.getTransaction().getUser() == null && myUser != null )
			||
			(e.getTransaction().getUser() != null && !e.getTransaction().getUser().equals(myUser)))
			{
