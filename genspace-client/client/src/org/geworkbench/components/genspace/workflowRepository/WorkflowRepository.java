...
/**
 * Added by Flavio
 * This UI class has been created by copying a considerable amount of code from
 * package org.geworkbench.engine.skin and the skin class in particular
 * It somehow overdesigned for our purpose, but it allows for extreme flexibility and extensibility
 * @author flavio
 *
 */
public class WorkflowRepository extends JPanel implements VisualPlugin, ActionListener, Runnable {
	...
	private void initComponents() throws Exception {
        ...
        jSplitPane2.setDividerLocation(GenSpace.getInstance().jframe.getHeight()*60/100);
        jSplitPane3.setDividerLocation(GenSpace.getInstance().jframe.getHeight()*55/100);
        ...
        repositoryDock.setComponentProvider(new ComponentProvider(REPOSITORY_AREA));
        
        
        repositoryPanel = new RepositoryPanel(this);
        addToContainer(REPOSITORY_AREA, repositoryPanel.getComponent(), "Repository", RepositoryPanel.class);
        graphPanel = new GraphPanel(this);
        addToContainer(VISUAL_AREA, graphPanel.getComponent(), "Workflow", GraphPanel.class);
        inboxTable = new InboxTablePanel(this);
        addToContainer(INBOX_AREA,inboxTable.getComponent(), "Inbox", InboxTablePanel.class);

        
        //DETAILS PANEL, add in inverse order of visualization
        workflowCommentsPanel = new WorkflowCommentsPanel(this);
        addToContainer(DETAILS_AREA, workflowCommentsPanel.getComponent(), "Workflow Comments", WorkflowCommentsPanel.class);
        workflowDetailsPanel = new WorkflowDetailsPanel(this);
        addToContainer(DETAILS_AREA, workflowDetailsPanel.getComponent(), "Workflow Details", WorkflowDetailsPanel.class);
    } 
