...
public class DynamicTree extends JPanel implements ActionListener, TreeSelectionListener {
    ...
    private void addWorkflowObject(WorkflowNode parent,
			UserWorkflow workflow, boolean visibile) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        org.jdesktop.swingworker.SwingWorker<Void, Void> worker = new org.jdesktop.swingworker.SwingWorker<Void, Void>() {
                ...
                if (currentSelection != null) {
                    ...
                    if (parent != null) {//Don't delete the root
                    	if(currentNode instanceof WorkflowNode){
         					...
         					String result = (String)ServerRequest.get(RuntimeEnvironmentSettings.WORKFLOW_REPOSITORY_SERVER, "delete_workflow", params);
         					if(result == null || result.equals("")){
                             	u.workflows.remove(uw);
                             	recalculateAndReload();
                             	repositoryPanel.workflowRepository.clearWorkflowData();
         					}
                    	}
                    	else{
                    		//removing a folder
                    		int option = JOptionPane.showConfirmDialog(null, "All workflows in the folder will be lost. Continue?");
                    		if(option == JOptionPane.YES_OPTION){
                    			String folderName = currentNode.getUserObject().toString();
                             	ArrayList<Object> params = new ArrayList<Object>();
             					params.add(u.username);
             					params.add(folderName);
             					String result = (String)ServerRequest.get(RuntimeEnvironmentSettings.WORKFLOW_REPOSITORY_SERVER, "delete_folder", params);
             					if(result == null || result.equals("")){
                                 	u.folders.remove(folderName);
                                 	Iterator<UserWorkflow> it = u.workflows.iterator();
                                 	while(it.hasNext()){
                                 		UserWorkflow uw = it.next();
                                 		if(uw.folder != null && uw.folder.equals(folderName))
                                 			it.remove();
                                 	}
                                 	recalculateAndReload();
             					}
			...
			private void addCommand(User u) {
            	String folderName = JOptionPane.showInputDialog("Input a folder name:");
            	if(folderName != null && !folderName.trim().equals("") || u.folders.contains(folderName)){
            		DefaultMutableTreeNode root = (DefaultMutableTreeNode)tree.getModel().getRoot();
            		if(folderName.equalsIgnoreCase(root.getUserObject().toString())){
            			folderName = null;
            		}
                    TreePath currentSelection = tree.getSelectionPath();
                    if (currentSelection != null) {
                        ...
                        if(currentNode instanceof WorkflowNode){
        					...
        					params.add(uw.workflow.ID);
        					params.add(folderName);
        					String result = (String)ServerRequest.get(RuntimeEnvironmentSettings.WORKFLOW_REPOSITORY_SERVER, "add_to_folder", params);
        					if(result == null || result.equals("")){
            		...
            		JOptionPane.showMessageDialog(null, "Input an existing folder name.");
            	}
			}
			...
			private void newCommand(User u) {
                ...
                //Add button clicked
            	String folderName = JOptionPane.showInputDialog("Select a folder name");
            	if(folderName != null && !folderName.trim().equals("") && !u.folders.contains(folderName)){
					// send add_folder to the server
            		ArrayList<Object> params = new ArrayList<Object>();
					params.add(u.username);
					params.add(folderName);
					String result = (String)ServerRequest.get(RuntimeEnvironmentSettings.WORKFLOW_REPOSITORY_SERVER, "new_folder", params);
					if(result == null || result.equals("")){
    ...
    /**
     * Invoked when a node of the tree is selected (the rename is performed by myTreeModeListener though.
     */
	public void valueChanged(TreeSelectionEvent e) {
		if(tree.getSelectionPath() != null && tree.getSelectionPath().getPathCount() >1){
			...
			if(node instanceof WorkflowNode){
				...
				repositoryPanel.workflowRepository.graphPanel.setAndPaintWorkflow(wf.userWorkflow.workflow);
				repositoryPanel.workflowRepository.workflowDetailsPanel.setAndPrintWorkflow(wf.userWorkflow.workflow);
				repositoryPanel.workflowRepository.workflowCommentsPanel.setData(wf.userWorkflow.workflow);
			}
		}
	}
}
