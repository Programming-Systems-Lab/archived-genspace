...
public class NetworkVisualization extends JPanel implements VisualPlugin,
		ActionListener {

	private static final String HOST = RuntimeEnvironmentSettings.NETWORK_VIS_SERVER
			.getHost();
	private static final int PORT = RuntimeEnvironmentSettings.NETWORK_VIS_SERVER
			.getPort();

	private String login = "swap"; // TODO: WHERE DOES THIS COME FROM?
	private String dir = "components/genspace/lib/"; // TODO: where to write the
														// XML files??
	...
	private void initComponents() {
		...
		getNetworkData(file);
		String prop = dir + "sample.properties"; // TODO: need to make sure this
													// file exists!
	...
	/**
	 * This actually does the work of connecting to the server and getting the
	 * network data. It writes the data to a file, and then returns the name of
	 * the file.
	 */
		...
		try {
			...
			while (in.hasNext()) {
				...
				if (line.startsWith("NETWORK")) {
					...
					outFile.println("<node name=\"" + currentNetwork
							+ "\"><label>" + currentNetwork + "</label></node>");
				}
				// if it's a user, create a node (if it doesn't already exist)
				// and add an edge
					...
					if (users.contains(user) == false) {
						...
						if (name.equals(login))
							name = "&lt;&lt;ME&gt;&gt;";
						outFile.println("<node name=\"" + user + "\"><label>"
								+ name + "</label></node>");
						users.add(user);
					}

					// now connect the user to the network
					outFile.println("<edge source=\"" + user + "\" target=\""
							+ currentNetwork + "\" />");
