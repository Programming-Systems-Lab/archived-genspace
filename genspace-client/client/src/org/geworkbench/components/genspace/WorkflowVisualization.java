		...
		org.jdesktop.swingworker.SwingWorker<Void, Void> worker = new org.jdesktop.swingworker.SwingWorker<Void, Void>() {
			...
			public Void doInBackground() {
				...
				try {
					...
					out.write(tool + "\n");
					// send the username and whether or not to limit to that
					// user's networks
					out.println(login);
				...
				/*
				 * This is just sample data!!!! if
				 * (action.equals("All workflows including")) { if
				 * (tool.equals("Log2 Normalizer")) { workflows.add(
				 * "Affymetrix Filter,Log2 Normalizer,T-test,Hierarchical Clustering,GO Terms"
				 * ); workflows.add(
				 * "Quantile Normalization,Log2 Normalizer,Aracne,Sequence Retriever,Promoter,MEDUSA"
				 * ); workflows.add(
				 * "Quantile Normalization,Log2 Normalizer,BLAST,Pattern Discovery"
				 * ); } else { workflows.add(
				 * "Quantile Normalization,Log2 Normalizer,BLAST,Pattern Discovery"
				 * ); workflows.add("BLAST,Pattern Discovery"); } } else if
				 * (action.equals("Most common workflow including")) { if
				 * (tool.equals("Log2 Normalizer")) workflows.add(
				 * "Quantile Normalization,Log2 Normalizer,Aracne,Sequence Retriever,Promoter,MEDUSA"
				 * ); else workflows.add(
				 * "Quantile Normalization,Log2 Normalizer,BLAST,Pattern Discovery"
				 * ); } else if
				 * (action.equals("Most common workflow starting with")) { if
				 * (tool.equals("Log2 Normalizer")) {
				 * 
				 * } else workflows.add("BLAST, Pattern Discovery"); }
				 */
				...
				// different method calls to handle the different types of
				// return values
				if (action.equals("All workflows including")) {
					...
					// if it's not all workflows, then it's just one, which
					// means we just have a single entry in the arraylist
