...
public class DataVisibility extends JPanel implements VisualPlugin,
	...
	public DataVisibility() {
		...
		try {
			...
			username = LoginManager.getLoggedInUser();
	...
	private void initComponents() {
		 ...
		 * dataVisibilityOptions.addItem
		 * ("-- Select Data Visibility Options --");
		 ...
		 * 
		 * ArrayList<String> allNetworks = getAllNetworks(); networks = new
		 * JList(allNetworks.toArray()); networks.setVisibleRowCount(3);
		 * networks
		 * .setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		 * networks.setEnabled(false); add(new JScrollPane(networks));
	...
	private void initComponents(DataVisibilityBean bean) {
		 ...
		 * 
		 * c.gridwidth = GridBagConstraints.REMAINDER;
		 * gridbag.setConstraints(blank, c); add(blank);
		 * 
		 * ArrayList<String> allNetworks = getAllNetworks(); networks = new
		 * JList(allNetworks.toArray()); networks.setVisibleRowCount(3);
		 * networks
		 * .setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
		 * if(bean.getDataVisibility() != 2) networks.setEnabled(false); else
		 * networks.setEnabled(true); List selectedNetworks =
		 * bean.getSelectedNetworks(); int[] selectedIndices = new
		 * int[selectedNetworks.size()] ; int j = 0; for(int i=0;
		 * i<allNetworks.size(); i++) {
		 * if(selectedNetworks.contains(allNetworks.get(i))) {
		 * selectedIndices[j]=i; j++; } } if (selectedNetworks.size() > 0)
		 * networks.setSelectedIndices(selectedIndices);
		 * 
		 * JScrollPane jp = new JScrollPane(networks); c.gridwidth =
		 * GridBagConstraints.REMAINDER; gridbag.setConstraints(jp, c); add(jp);
		 * networks.addListSelectionListener(this);
		 * 
		 * c.gridwidth = GridBagConstraints.REMAINDER;
		 * gridbag.setConstraints(blank, c); add(blank);
		...
		if (e.getSource() == save) {
			...
			javax.swing.SwingWorker<Void, Void> worker = new javax.swing.SwingWorker<Void, Void>() {
				...
				public Void doInBackground() {
					 ...
					 * (dataVisibilityOptions.getSelectedIndex() == 3 &&
					 ...
					 */
						...
						dvb.setLogData((short) (logPreferences
								.getSelectedIndex() - 1));
	...
	public void valueChanged(ListSelectionEvent e) {
		selectedNetworks = Arrays.asList(networks.getSelectedValues());
