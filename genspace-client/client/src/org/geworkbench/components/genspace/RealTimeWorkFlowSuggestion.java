...
public class RealTimeWorkFlowSuggestion extends JPanel implements VisualPlugin,
	...
	public static ArrayList<TransactionElement> currentWorkFlow = new ArrayList();
	private static String currentTid = null;

	private static boolean firstTimeFlag = true;// used for checking the
												// beginning of new transactions
	...
	private static JScrollPane workflowViewerScrollPane = new JScrollPane(
			workflowViewerPanel);
	...
	public static void displayCWF() {
		...
		// ---------------
		String cwfSeparate = "";// to store the details of the current user
								// transaction
		for (int i = 0; i < currentWorkFlow.size(); i++) {
			cwfSeparate = cwfSeparate + "\t\t"
					+ (currentWorkFlow.get(i)).getHour() + ":"
					+ (currentWorkFlow.get(i)).getMinute() + ":"
					+ (currentWorkFlow.get(i)).getSecond() + " "
					+ (currentWorkFlow.get(i)).getToolName() + "\n";
		...
		for (int i = 0; i < usedWorkFlowToday.size(); i++) {
			...
			String tempWF = usedWorkFlowToday.get(i);

			/*
			 * for (int j = 0; j < tempWF.size(); j ++) {
			 * System.out.print(((TransactionElement
			 * )tempWF.get(j)).getToolName() + " -> "); }
			 */
		...
		// ----------------------
		// Now we use cwfString to talk to ISBUServer to get real time
		// suggestion
		try {
			ArrayList RealTimeWFSuggestionResults = getRealTimeWorkFlowSuggestion(cwfString);

			// get statA
			statAValue = ((Integer) RealTimeWFSuggestionResults.get(0))
					.intValue();
		...
		for (int i = 0; i < statBValue.size(); i++) {
			...
			statBDisplay = statBDisplay
					+ superWF.substring(0, superWF.length() - 1) + "   ("
					+ times + " times)\n";
		...
		String statCDisplay = "";
		// we need to first go through statCValue once to calculate the total
		// times...
		float totalTimes = 0;
		for (int j = 0; j < statCValue.size(); j++) {
			String temp = statCValue.get(j);
		...
		for (int i = 0; i < statCValue.size(); i++) {
			String temp = statCValue.get(i);

			StringTokenizer tokens = new StringTokenizer(temp, "#");

			String superWF = tokens.nextToken();
			float times = Integer.parseInt(tokens.nextToken());
		...
		/*
		 * //------ String finishedWF = ""; for (int i = 0; i <
		 * usedWorkFlowToday.size(); i ++) { ArrayList tempWF =
		 * (ArrayList)usedWorkFlowToday.get(i);
		 * 
		 * for (int j = 0; j < tempWF.size(); j ++) { finishedWF = finishedWF +
		 * tempWF.get(j) + "->"; } }
		 */
		...
		infoArea.append("Your current work flow has been used " + statAValue
				+ " times in history." + "\n\n");
		...
		String nextBestRated = ServerRequest.get(
				RuntimeEnvironmentSettings.ISBU_SERVER, "nextBestRatedTool",
				arguments).toString();
	...
	public static void updateCWFStatus(final int hour, final int minute,
		...
		org.jdesktop.swingworker.SwingWorker<Void, Void> worker = new org.jdesktop.swingworker.SwingWorker<Void, Void>() {
			...
			public Void doInBackground() {
				...
				// first we check that whether we have started a new transaction
				// (a new work flow)
				if (firstTimeFlag == true) { // we are about to begin the first
												// transaction today, now
												// currentTid is still "null",
												// rather than an earlier value
					currentTid = transactionID;
					TransactionElement element = new TransactionElement(hour,
							minute, second, toolName);
					...
					if (transactionID.equals(currentTid)) { // we are still in
															// the same
															// transaction, just
															// go ahead

						// System.out.println(" ");
						// System.out.println("Incoming Transaction ID: " +
						// transactionID);
						// System.out.println("current Transaction ID: " +
						// currentTid);
						// System.out.println(" ");

						TransactionElement element = new TransactionElement(
								hour, minute, second, toolName);
				...
				WorkflowViewerPanelNode newNode = new WorkflowViewerPanelNode(
						toolName, currentWorkFlow.size() - 1);
	...
	private static ArrayList getRealTimeWorkFlowSuggestion(String cwf)
			throws Exception {

		// System.out.println(" ");
		// System.out.println("Current CWF being sent: " + cwf);
		clientSideID = "RTWFS#getRealTimeWorkFlowSuggestion#" + cwf;// zhu yi
																	// kong ge
																	// shi zi
																	// dai de
																	// ...
	...
	public static void main(String args[]) {
		...
		RealTimeWorkFlowSuggestion.updateCWFStatus(10, 20, 30, "ARACNE",
				"ABCDEFG");
		RealTimeWorkFlowSuggestion.updateCWFStatus(10, 20, 30, "ARACNE",
				"ABCDEFG");
