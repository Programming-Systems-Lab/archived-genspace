...
import org.geworkbench.components.simulation.Reaction.ReactionInfo;
import org.geworkbench.components.simulation.Reaction.GeneInfo;
import java.io.*;
import javax.swing.*;
import java.awt.*;
...
public class SimulatorCytoscape extends EventSource implements VisualPlugin, MenuListener {
    ...
    public void getGeneInfo() {
        this.geneInfo = sim.getGeneInfo();
        geneCounter = geneInfo.size();
    }

    public void getSaveFile(String saveFile) {
        this.saveFile = saveFile;
    }

    public void setSim(SimulationMain sim) {
        this.sim = sim;
    }

    public void clearGenes() {
        geneInfo = new ArrayList<GeneInfo>();
        geneCounter = 0;
        Cytoscape.destroyNetworkView(Cytoscape.getCurrentNetworkView());
        Cytoscape.destroyNetwork(cytoNetwork);
        createNewNetwork();
        Cytoscape.createNetworkView(cytoNetwork);
    }

    public void addGene(double deg, double conc) {
        GeneInfo foo = new GeneInfo("G" + geneCounter + "m");
        foo.setDeg(deg);
        foo.setConcentration(conc);
        geneCounter++;
        geneInfo.add(foo);
        updateView();
        printGeneInfo();
    ...
    public int getGeneIndex(String name) {
        for (int i = 0; i < geneInfo.size(); i++) {
            if (geneInfo.get(i).getGeneName().contains(name)) {
                return i;
            }
        }
        return -1;
    ...
    public void removeGene(String name) {
        int removeIndex = getGeneIndex(name);
        if (removeIndex != -1) {
            geneInfo.remove(removeIndex);
        }

        for (int i = 0; i < geneInfo.size(); i++) {
            removeGeneEdge(geneInfo.get(i).getGeneName(), name);
        }
    ...
    public void removeGeneEdge(String target, String source) {
        int index = getGeneIndex(target);
        if (index != -1) {
            geneInfo.get(index).removeSource(source);
        }
        updateView();
        printGeneInfo();
    }
    ...
    public void printGeneInfo(){
        for(int i = 0; i < geneInfo.size(); i++) {
            System.out.println("GENE " + geneInfo.get(i).getGeneName());
            for (int j = 0; j < geneInfo.get(i).getSources().size(); j++) {
                System.out.println("SOURCE" + j + " " + geneInfo.get(i).getSources().get(j).getSource());
            }
        }
    }

    public void createGeneEdge(String target, String source, double k, double hill, double vmax, String interaction) {
        int index = getGeneIndex(target);
        GeneInfo foo = geneInfo.get(index);
        foo.addSource(source, k, hill, vmax, interaction);
        updateView();
        printGeneInfo();
    }

    public void createSBMLFile(String fileName) {
        sim.generateGeneInfoNetwork(geneInfo, fileName);
    }
    ...
    public class CyWindowFake implements CyWindow{
        public CyWindowFake() {}
    ...
    public void updateView() {
        //simCounter--;
        cytoNetwork = Cytoscape.getCurrentNetwork();

        for (int i = 0; i < cytoNetwork.getEdgeCount(); i++) {
            cytoNetwork.removeEdge(i,true);
        }

        for (int i = 0; i < cytoNetwork.getNodeCount(); i++) {
            cytoNetwork.removeNode(i,true);
        }
        ...
        CyWindow poo = new CyWindowFake();
        YFilesLayoutPlugin plugin = new YFilesLayoutPlugin(poo);
        OrganicLayout organiclayout = new OrganicLayout(plugin);
        organiclayout.actionPerformed(null);


        //cytoscape.layout.LayoutAlgorithm layout = new CircleGraphLayout(Cytoscape.getCurrentNetworkView());
    ...
    public void createNewNetwork() {
        cytoNetwork = Cytoscape.createNetwork("sim " + simCounter);
        simCounter++;
    }
    public void updateNetwork() {
        EdgeAppearance actEdge = new EdgeAppearance();
        actEdge.setSourceArrow(Arrow.BLACK_CIRCLE);
        EdgeAppearance inhEdge = new EdgeAppearance();
        inhEdge.setSourceArrow(Arrow.BLACK_T);

         ArrayList<CyEdge> allActEdges = new ArrayList<CyEdge>();
         ArrayList<CyEdge> allInhEdges = new ArrayList<CyEdge>();
         ArrayList<CyNode> allNodes = new ArrayList<CyNode>();
         for (int i = 0; i < geneInfo.size(); i++) {
            GeneInfo gene = (GeneInfo)geneInfo.get(i);
            ArrayList<ReactionInfo> reactionInfo = (ArrayList<ReactionInfo>)gene.getSources();
            CyNode n2 = Cytoscape.getCyNode(gene.getGeneName(), true);
            cytoNetwork.addNode(n2);
            allNodes.add(n2);
            for(int j = 0; j < reactionInfo.size(); j++) {
                ReactionInfo reaction = (ReactionInfo)reactionInfo.get(j);
                CyEdge e = null;
                CyNode n1 = Cytoscape.getCyNode(reaction.getSource(), true);
                cytoNetwork.addNode(n1);
                allNodes.add(n1);

                if (reaction.getInteraction().contains("Activator")) {
                    e = Cytoscape.getCyEdge(reaction.getSource(), reaction.getSource() + " activates " + gene.getGeneName(), gene.getGeneName(), "pp");
                    cytoNetwork.addEdge(e);
                    allActEdges.add(e);
                } else {
                    e = Cytoscape.getCyEdge(reaction.getSource(), reaction.getSource() + " inhibits " + gene.getGeneName(), gene.getGeneName(), "pd");
                    cytoNetwork.addEdge(e);
                    allInhEdges.add(e);
                }
            }
        ...
        Cytoscape.createNetworkView(cytoNetwork);
        for (int i = 0; i < allActEdges.size(); i++) {
            Cytoscape.getCurrentNetworkView().getEdgeView(allActEdges.get(i)).setTargetEdgeEnd(EdgeView.BLACK_ARROW);
        }
        for (int i = 0; i < allInhEdges.size(); i++) {
//            Cytoscape.getCurrentNetworkView().getEdgeView(allInhEdges.get(i)).setSelectedPaint(Color.RED);
            Cytoscape.getCurrentNetworkView().getEdgeView(allInhEdges.get(i)).setTargetEdgeEnd(EdgeView.EDGE_COLOR_T);
        }
        for (int i = 0; i < allNodes.size(); i++) {
            Cytoscape.getCurrentNetworkView().getNodeView(allNodes.get(i)).setShape(NodeView.ELLIPSE);
        }
    }

    public SimulatorCytoscape() {
    ...
    /**
     * try to draw the network with the interaction type
     */
//    public void drawNetworkAndInteraction(AdjacencyMatrixEvent ae) {
//        adjMatrix = (EvdAdjacencyMatrix) ae.getAdjacencyMatrix();
//        createWholeNetwork();
//    }

    /**
     * the current way to draw interaction networks with labeled edges <BR>
     * createSubNetwork is used for MI-generated networks
     */
//    private void createWholeNetwork() {
//
//        EvdAdjacencyMatrix tmpadj = (EvdAdjacencyMatrix) this.adjMatrix;
//        Object[] tmpFilter = tmpadj.getFilter();
//
//        double threshold = 0.0;
//        boolean passedFilter = false;
//        double mistrength = -1;
//
//        for (int i = 0; i < tmpadj.getNumType2Interaction(); i++) {
//            int geneId = tmpadj.getInteractionType2GeneAID(i);
//
//            DSGeneMarker gm1 = (DSGeneMarker) maSet.getMarkers().get(geneId);
//            int gm1Size = tmpadj.getConnectionNo(gm1.getSerial(), threshold);
//
//            int geneid2 = tmpadj.getInteractionType2GeneBID(i);
//
//            if ((geneid2 < maSet.getMarkers().size()) && (geneid2 != 12600)) {
//
//                String actiontype = tmpadj.getInteractionType2Action(i);
//                mistrength = tmpadj.getInteractionType2Strength(geneId, geneid2);
//
//                // test is the edge is strong enough
//                if (mistrength <= threshold) {
//                    continue;
//                }
//
//                passedFilter = false;
//
//                // test the interaction type to see if it is the desired interaction
//                // we may also test if the edge is strong enough to be shown here...
//                // this is to be compatable with the MI-generated networks
//                if (tmpFilter == null || tmpFilter.length == 0) {
//                    passedFilter = true;
//                } else if (actiontype.equals("pp") || tmpFilter[0].equals("ALL")) {
//                    passedFilter = true;
//                } else {
//                    for (int j = 0; j < tmpFilter.length; j++) {
//                        if (tmpFilter[j].equals(actiontype)) {
//                            passedFilter = true;
//                            break;
//                        }
//                    }
...
//                if (thresholdTest) {
//
//                    DSGeneMarker gm2 = (DSGeneMarker) maSet.getMarkers().get(geneid2);
//                    int gm2Size = tmpadj.getConnectionNo(gm2.getSerial(), threshold);
//
//                    String geneName1 = gm1.getShortName();
//                    String geneName2 = gm2.getShortName();
//
//                    // comment copied from createSubNetwork
...
//                        // set up the edge
//                        CyEdge e = null;
//                        e = Cytoscape.getCyEdge(g1Name, g1Name + "." + actiontype + "." + g2Name, g2Name, actiontype);
//                        cytoNetwork.addEdge(e);
//                    } else {
//                    }
//                }
...
//    }
     ...
     *             create the subnetwork and label or filter the org.geworkbench.components.interactions.cellularnetwork <BR>
