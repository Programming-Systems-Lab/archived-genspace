...
//*
        ...
        if (metrop == 0) {
            ...
            generateGenes3(mrna, protein, activepro, m);
        }
...
//                System.out.println("fanIn%: " + (fanIn[i] / (fanIn[i] + fanOut[i])) + " total:" + total);
    ...
    void generateLoopySFN(Species[] genes, Model m) {
        ...
        for (int i = 0; i < numConnections; i++) {
            ...
            numCons[gene1]++;
            numCons[gene2]++;
            System.out.println("fanIn percent: " + calculateFanInPercent(fanIn, fanOut) + " " + fanInPercent);
        }
        for (int i = 0; i < numGenes; i++) {
            System.out.print(fanIn[i] + " ");
        }
        System.out.flush();
        System.out.println("");
        for (int i = 0; i < numGenes; i++) {
            System.out.print(fanOut[i] + " ");
        }
        System.out.flush();
