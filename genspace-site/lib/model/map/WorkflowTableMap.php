<?php



/**
 * This class defines the structure of the 'WORKFLOW' table.
 *
 *
 *
 * This map class is used by Propel to do runtime db structure discovery.
 * For example, the createSelectSql() method checks the type of a given column used in an
 * ORDER BY clause to know whether it needs to apply SQL to make the ORDER BY case-insensitive
 * (i.e. if it's a text column type).
 *
 * @package    propel.generator.lib.model.map
 */
class WorkflowTableMap extends TableMap {

	/**
	 * The (dot-path) name of this class
	 */
	const CLASS_NAME = 'lib.model.map.WorkflowTableMap';

	/**
	 * Initialize the table attributes, columns and validators
	 * Relations are not initialized by this method since they are lazy loaded
	 *
	 * @return     void
	 * @throws     PropelException
	 */
	public function initialize()
	{
	  // attributes
		$this->setName('WORKFLOW');
		$this->setPhpName('Workflow');
		$this->setClassname('Workflow');
		$this->setPackage('lib.model');
		$this->setUseIdGenerator(true);
		// columns
		$this->addPrimaryKey('ID', 'Id', 'INTEGER', true, 11, null);
		$this->addColumn('USAGECOUNT', 'Usagecount', 'INTEGER', false, 11, null);
		$this->addColumn('CREATEDAT', 'Createdat', 'TIMESTAMP', false, null, null);
		$this->addColumn('CREATOR_ID', 'CreatorId', 'INTEGER', false, 11, null);
		$this->addColumn('PARENT_ID', 'ParentId', 'INTEGER', false, 11, null);
		$this->addColumn('CREATIONTRANSACTION_ID', 'CreationtransactionId', 'INTEGER', false, 11, null);
		$this->addColumn('NUMRATING', 'Numrating', 'INTEGER', false, 11, null);
		$this->addColumn('SUMRATING', 'Sumrating', 'INTEGER', false, 11, null);
		$this->addColumn('LEGACY_ID', 'LegacyId', 'INTEGER', false, 11, null);
		// validators
	} // initialize()

	/**
	 * Build the RelationMap objects for this table relationships
	 */
	public function buildRelations()
	{
	} // buildRelations()

} // WorkflowTableMap
