...
abstract class BaseAudit extends BaseObject  implements Persistent
{
  ...
  const PEER = 'AuditPeer';
	...
	/**
	 * The value for the beforevalue field.
	 * @var        string
	 */
	protected $beforevalue;

	/**
	 * The value for the aftervalue field.
	 * @var        string
	 */
	...
	/**
	 * Get the [beforevalue] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Get the [aftervalue] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Set the value of [beforevalue] column.
	 * 
	 * @param      string $v new value
	 * @return     Audit The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [aftervalue] column.
	 * 
	 * @param      string $v new value
	 * @return     Audit The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->tablename = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->beforevalue = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->aftervalue = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			...
			return $startcol + 7; // 7 = AuditPeer::NUM_COLUMNS - AuditPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AuditPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AuditPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AuditPeer::ID) ) {
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Audit (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Audit Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getRegistration(PropelPDO $con = null)
	{
		if ($this->aRegistration === null && (($this->username !== "" && $this->username !== null))) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aRegistration->addAudits($this);
			 */
		}
