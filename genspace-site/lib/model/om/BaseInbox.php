...
/**
 * Base class that represents a row from the 'Inbox' table.
 *
 * 
 *
...
abstract class BaseInbox extends BaseObject  implements Persistent
{
  ...
  const PEER = 'InboxPeer';
	 ...
	 * @return     Inbox The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = InboxPeer::NUM_COLUMNS - InboxPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = InboxPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				InboxPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(InboxPeer::MESSAGEID) ) {
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Inbox (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Inbox Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function setRegistrationRelatedByFromuser(Registration $v = null)
	{
		...
		if ($v !== null) {
			$v->addInboxRelatedByFromuser($this);
		}
	...
	public function getRegistrationRelatedByFromuser(PropelPDO $con = null)
	{
		if ($this->aRegistrationRelatedByFromuser === null && (($this->fromuser !== "" && $this->fromuser !== null))) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aRegistrationRelatedByFromuser->addInboxsRelatedByFromuser($this);
			 */
		}
	...
	public function setRegistrationRelatedByTouser(Registration $v = null)
	{
		...
		// If this object has already been added to the Registration object, it will not be re-added.
		if ($v !== null) {
			$v->addInboxRelatedByTouser($this);
		}
	...
	public function getRegistrationRelatedByTouser(PropelPDO $con = null)
	{
		if ($this->aRegistrationRelatedByTouser === null && (($this->touser !== "" && $this->touser !== null))) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aRegistrationRelatedByTouser->addInboxsRelatedByTouser($this);
			 */
		}
