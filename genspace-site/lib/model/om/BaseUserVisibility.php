...
/**
 * Base class that represents a row from the 'user_visibility' table.
 *
 * 
 *
...
abstract class BaseUserVisibility extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
  const PEER = 'UserVisibilityPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        UserVisibilityPeer
	 */
	...
	/**
	 * The value for the uservisibility field.
	 * @var        int
	 */
	...
	/**
	 * Get the [uservisibility] column value.
	 * 
	 * @return     int
	 */
	 ...
	 * @return     UserVisibility The current object (for fluent API support)
	 */
	...
	/**
	 * Set the value of [uservisibility] column.
	 * 
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->uservisibility = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			...
			return $startcol + 3; // 3 = UserVisibilityPeer::NUM_COLUMNS - UserVisibilityPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UserVisibilityPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				UserVisibilityPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(UserVisibilityPeer::ID) ) {
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UserVisibility (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UserVisibility Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getRegistration(PropelPDO $con = null)
	{
		if ($this->aRegistration === null && (($this->username !== "" && $this->username !== null))) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aRegistration->addUserVisibilitys($this);
			 */
		}
