...
abstract class BaseHistory extends BaseObject  implements Persistent
{
  ...
  const PEER = 'HistoryPeer';
	...
	/**
	 * The value for the accessedat field.
	...
	/**
	 * Get the [optionally formatted] temporal [accessedat] column value.
	 * 
	 *
	...
	public function getAccessedat($format = 'Y-m-d H:i:s')
	{
		...
		try {
			$dt = new DateTime($this->accessedat);
		} catch (Exception $x) {
			throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->accessedat, true), $x);
		}
	...
	public function setWspid($v)
	{
		...
		if ($this->wspid !== $v) {
			$this->wspid = $v;
			$this->modifiedColumns[] = HistoryPeer::WSPID;
		}
	...
	public function setUid($v)
	{
		...
		if ($this->uid !== $v) {
			$this->uid = $v;
			$this->modifiedColumns[] = HistoryPeer::UID;
		}
	...
	/**
	 * Sets the value of [accessedat] column to a normalized version of the date/time value specified.
	 * 
	...
	public function setAccessedat($v)
	{
		...
		if ( $this->accessedat !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->accessedat !== null && $tmpDt = new DateTime($this->accessedat)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->accessedat = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = HistoryPeer::ACCESSEDAT;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->accessedat = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(HistoryPeer::ID) ) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setAccessedat($arr[$keys[4]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(HistoryPeer::WSPID)) $criteria->add(HistoryPeer::WSPID, $this->wspid);
		if ($this->isColumnModified(HistoryPeer::UID)) $criteria->add(HistoryPeer::UID, $this->uid);
		if ($this->isColumnModified(HistoryPeer::TYPE)) $criteria->add(HistoryPeer::TYPE, $this->type);
		if ($this->isColumnModified(HistoryPeer::ACCESSEDAT)) $criteria->add(HistoryPeer::ACCESSEDAT, $this->accessedat);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setAccessedat($this->accessedat);
	...
	public function getWorkspace(PropelPDO $con = null)
	{
		if ($this->aWorkspace === null && ($this->wspid !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aWorkspace->addHistorys($this);
			 */
		}
	...
	public function getRegistration(PropelPDO $con = null)
	{
		if ($this->aRegistration === null && ($this->uid !== null)) {
			...
			/* The following can be used additionally to
			   ...
			   undesirable since it could result in an only partially populated collection
			   in the referenced object.
			   $this->aRegistration->addHistorys($this);
			 */
		}
