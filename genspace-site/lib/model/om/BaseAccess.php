 ...
 */
abstract class BaseAccess extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
  const PEER = 'AccessPeer';
	 ...
	 * @return     Access The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = AccessPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = AccessPeer::NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = AccessPeer::NUM_COLUMNS - AccessPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Access object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AccessPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AccessPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AccessPeer::ID) ) {
					...
					$affectedRows = AccessPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AccessPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = AccessPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = AccessPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AccessPeer::ID)) $criteria->add(AccessPeer::ID, $this->id);
		if ($this->isColumnModified(AccessPeer::NAME)) $criteria->add(AccessPeer::NAME, $this->name);
	...
	{
		$criteria = new Criteria(AccessPeer::DATABASE_NAME);
		$criteria->add(AccessPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Access (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Access Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this Access is new, it will return
	 ...
	 * Otherwise if this Access is new, it will return
	 * an empty collection; or if this Access has previously
	 * been saved, it will retrieve related WorkspaceUsers from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Access.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
