...
/**
 * Base class that represents a row from the 'data_visibility' table.
 *
 * 
 *
...
abstract class BaseDataVisibility extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
  const PEER = 'DataVisibilityPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        DataVisibilityPeer
	 */
	 ...
	 * @return     DataVisibility The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->logdata = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->datavisibility = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = DataVisibilityPeer::NUM_COLUMNS - DataVisibilityPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = DataVisibilityPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				DataVisibilityPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of DataVisibility (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     DataVisibility Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getRegistration(PropelPDO $con = null)
	{
		if ($this->aRegistration === null && (($this->username !== "" && $this->username !== null))) {
			...
			$this->aRegistration->setDataVisibility($this);
		}
