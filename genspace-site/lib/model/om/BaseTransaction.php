...
abstract class BaseTransaction extends BaseObject  implements Persistent
{
  ...
  const PEER = 'TransactionPeer';
	...
	/**
	 * The value for the datasetname field.
	 * @var        string
	 */
	...
	/**
	 * Get the [datasetname] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Set the value of [datasetname] column.
	 * 
	 * @param      string $v new value
	 * @return     Transaction The current object (for fluent API support)
	 */
	public function setDatasetname($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->datasetname !== $v) {
			$this->datasetname = $v;
			$this->modifiedColumns[] = TransactionPeer::DATASETNAME;
		}
	...
	public function setClientid($v)
	{
		...
		if ($this->clientid !== $v) {
			$this->clientid = $v;
			$this->modifiedColumns[] = TransactionPeer::CLIENTID;
		}
	...
	public function setHostname($v)
	{
		...
		if ($this->hostname !== $v) {
			$this->hostname = $v;
			$this->modifiedColumns[] = TransactionPeer::HOSTNAME;
		}
	...
	public function setDate($v)
	{
		...
		if ( $this->date !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = TransactionPeer::DATE;
			}
	...
	public function setWorkflowId($v)
	{
		...
		if ($this->workflow_id !== $v) {
			$this->workflow_id = $v;
			$this->modifiedColumns[] = TransactionPeer::WORKFLOW_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->datasetname = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->clientid = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			...
			$this->workflow_id = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->resetModified();
			...
			return $startcol + 7; // 7 = TransactionPeer::NUM_COLUMNS - TransactionPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(TransactionPeer::ID) ) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setDatasetname($arr[$keys[1]]);
		...
		if (array_key_exists($keys[6], $arr)) $this->setWorkflowId($arr[$keys[6]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TransactionPeer::DATASETNAME)) $criteria->add(TransactionPeer::DATASETNAME, $this->datasetname);
		if ($this->isColumnModified(TransactionPeer::CLIENTID)) $criteria->add(TransactionPeer::CLIENTID, $this->clientid);
		if ($this->isColumnModified(TransactionPeer::HOSTNAME)) $criteria->add(TransactionPeer::HOSTNAME, $this->hostname);
		if ($this->isColumnModified(TransactionPeer::DATE)) $criteria->add(TransactionPeer::DATE, $this->date);
		if ($this->isColumnModified(TransactionPeer::USER_ID)) $criteria->add(TransactionPeer::USER_ID, $this->user_id);
		if ($this->isColumnModified(TransactionPeer::WORKFLOW_ID)) $criteria->add(TransactionPeer::WORKFLOW_ID, $this->workflow_id);
	...
	{
		$copyObj->setDatasetname($this->datasetname);
