...
/**
 * Base class that represents a row from the 'tig_nodes' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BaseTigNodes extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
  const PEER = 'TigNodesPeer';
	...
	/**
	 * The value for the parent_nid field.
	 * @var        string
	 */
	protected $parent_nid;
	...
	/**
	 * Get the [parent_nid] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     TigNodes The current object (for fluent API support)
	 */
	public function setNid($v)
	{
		...
		if ($this->nid !== $v) {
			$this->nid = $v;
			$this->modifiedColumns[] = TigNodesPeer::NID;
		}
	...
	/**
	 * Set the value of [parent_nid] column.
	 * 
	 * @param      string $v new value
	 * @return     TigNodes The current object (for fluent API support)
	 */
	public function setParentNid($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->parent_nid !== $v) {
			$this->parent_nid = $v;
			$this->modifiedColumns[] = TigNodesPeer::PARENT_NID;
		}
	...
	public function setUid($v)
	{
		...
		if ($this->uid !== $v) {
			$this->uid = $v;
			$this->modifiedColumns[] = TigNodesPeer::UID;
		}
	...
	public function setNode($v)
	{
		...
		if ($this->node !== $v) {
			$this->node = $v;
			$this->modifiedColumns[] = TigNodesPeer::NODE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->parent_nid = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->uid = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->node = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = TigNodesPeer::NUM_COLUMNS - TigNodesPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating TigNodes object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = TigNodesPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				TigNodesPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(TigNodesPeer::NID) ) {
					...
					$affectedRows += 1;
					$this->setNid($pk);  //[IMV] update autoincrement primary key
					$this->setNew(false);
				} else {
					$affectedRows += TigNodesPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = TigNodesPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = TigNodesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = TigNodesPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getNid(),
			$keys[1] => $this->getParentNid(),
			$keys[2] => $this->getUid(),
			$keys[3] => $this->getNode(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setParentNid($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setUid($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setNode($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TigNodesPeer::NID)) $criteria->add(TigNodesPeer::NID, $this->nid);
		if ($this->isColumnModified(TigNodesPeer::PARENT_NID)) $criteria->add(TigNodesPeer::PARENT_NID, $this->parent_nid);
		if ($this->isColumnModified(TigNodesPeer::UID)) $criteria->add(TigNodesPeer::UID, $this->uid);
		if ($this->isColumnModified(TigNodesPeer::NODE)) $criteria->add(TigNodesPeer::NODE, $this->node);
	...
	{
		$criteria = new Criteria(TigNodesPeer::DATABASE_NAME);
		$criteria->add(TigNodesPeer::NID, $this->nid);
	...
	/**
	 * Generic method to set the primary key (nid column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of TigNodes (or compatible) type.
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @throws     PropelException
	 */
	public function copyInto($copyObj, $deepCopy = false)
	{
		$copyObj->setParentNid($this->parent_nid);
		$copyObj->setUid($this->uid);
		$copyObj->setNode($this->node);
		...
		$copyObj->setNew(true);
		$copyObj->setNid(NULL); // this is a auto-increment column, so set to default value
	}
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     TigNodes Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getTigUsers(PropelPDO $con = null)
	{
		if ($this->aTigUsers === null && (($this->uid !== "" && $this->uid !== null))) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aTigUsers->addTigNodess($this);
			 */
		}
	 ...
	 * If this TigNodes is new, it will return
	 ...
	 * Otherwise if this TigNodes is new, it will return
	 * an empty collection; or if this TigNodes has previously
	 * been saved, it will retrieve related TigPairss from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in TigNodes.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 ...
	 * @return     PropelCollection|array TigPairs[] List of TigPairs objects
	 */
	public function getTigPairssJoinTigUsers($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = TigPairsQuery::create(null, $criteria);
		$query->joinWith('TigUsers', $join_behavior);
