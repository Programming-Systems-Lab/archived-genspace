...
abstract class BaseSetting extends BaseObject  implements Persistent
{
  ...
  const PEER = 'SettingPeer';
	...
	/**
	 * The value for the data_value field.
	 * @var        string
	 */
	...
	/**
	 * Get the [data_value] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Set the value of [data_value] column.
	 * 
	...
	public function setDataValue($v)
	{
		...
		if ($this->data_value !== $v) {
			$this->data_value = $v;
			$this->modifiedColumns[] = SettingPeer::DATA_VALUE;
		}
	...
	public function setDataKey($v)
	{
		...
		if ($this->data_key !== $v) {
			$this->data_key = $v;
			$this->modifiedColumns[] = SettingPeer::DATA_KEY;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->data_value = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->data_key = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->resetModified();
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setDataValue($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setDataKey($arr[$keys[2]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SettingPeer::DATA_VALUE)) $criteria->add(SettingPeer::DATA_VALUE, $this->data_value);
		if ($this->isColumnModified(SettingPeer::DATA_KEY)) $criteria->add(SettingPeer::DATA_KEY, $this->data_key);
	...
	{
		$copyObj->setDataValue($this->data_value);
		$copyObj->setDataKey($this->data_key);
