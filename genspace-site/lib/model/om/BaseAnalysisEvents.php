...
/**
 * Base class that represents a row from the 'analysis_events' table.
 *
 * 
 *
...
abstract class BaseAnalysisEvents extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
  const PEER = 'AnalysisEventsPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        AnalysisEventsPeer
	 */
	 ...
	 * @return     AnalysisEvents The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 8; // 8 = AnalysisEventsPeer::NUM_COLUMNS - AnalysisEventsPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AnalysisEventsPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AnalysisEventsPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(AnalysisEventsPeer::ID) ) {
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of AnalysisEvents (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     AnalysisEvents Clone of current object.
	 * @throws     PropelException
	 */
