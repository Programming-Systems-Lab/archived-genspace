...
/**
 * Base class that represents a row from the 'include_exclude_analysis' table.
 *
 * 
 *
...
abstract class BaseIncludeExcludeAnalysis extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
  const PEER = 'IncludeExcludeAnalysisPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        IncludeExcludeAnalysisPeer
	 */
	 ...
	 * @return     IncludeExcludeAnalysis The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->analysis = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			...
			return $startcol + 4; // 4 = IncludeExcludeAnalysisPeer::NUM_COLUMNS - IncludeExcludeAnalysisPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = IncludeExcludeAnalysisPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				IncludeExcludeAnalysisQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				IncludeExcludeAnalysisPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(IncludeExcludeAnalysisPeer::ID) ) {
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of IncludeExcludeAnalysis (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     IncludeExcludeAnalysis Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     IncludeExcludeAnalysisPeer
	 */
