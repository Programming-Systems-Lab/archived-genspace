...
import org.geworkbenchweb.components.genspace.wrapper.WorkflowWrapper;
...
public class WorkflowStatistics extends CustomComponent implements GenSpaceTab {
	...
	{
		int i =1;
		for(Tool t: GenSpaceServerFactory.getUsageOps().getToolsByPopularity())
		{
		...
		i =1;
		for(Tool t: GenSpaceServerFactory.getUsageOps().getMostPopularWFHeads())
		{
			Label l = new Label(i+": " + t.getName());
		...
		i =1;
		for(Workflow s: GenSpaceServerFactory.getUsageOps().getWorkflowsByPopularity())
		{
	...
	private VerticalLayout buildIndividualToolStatisticsContainer() {
		...
		cmbSelectedTool.setImmediate(false);
