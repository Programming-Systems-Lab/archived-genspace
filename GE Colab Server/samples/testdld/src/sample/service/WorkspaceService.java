...
public class WorkspaceService {
	...
	private OMElement createDelUsrWorkspaceElement(String userinfo) throws Exception {
		...
		String query = "delete from registration where id=" + uid;
		System.out.println(query);
		stmt.executeUpdate(query);
	...
	private OMElement createStatusWorkspaceElement(String projectName) throws Exception {
		...
		query = "select w.locked, r.username, w.lastSync, r.first_name, r.last_name from workspace w, registration r where w.lastLockedUser=r.id and w.id="+id;
		try{
		    rs = stmt.executeQuery(query);
		    if (rs.next()){
	...
	private OMElement createLockWorkspaceElement(String projectName) throws Exception {
		...
		String query = "update workspace set createdAt=createdAt, lastSync=lastSync, locked=TRUE, lastLockedUser="+uid+" where id="+id;
		System.out.println(query);
		stmt.executeUpdate(query);
	...
	private OMElement createInfoWorkspaceElement(String projectName) throws RemoteException {
		...
		if (access==null || access.equals("read") || access.equals("write"))
			...
			try {
				...
				rs = stmt.executeQuery("select r.username, h.type, h.accessedAt, r.first_name, r.last_name from history h, registration r where h.uid = r.id and h.wspid = "+id+" order by h.accessedAt desc");
				while (rs.next()) {
		...
		try {
			...
			int count = rs.next() ? rs.getInt(1) : 0;
			wspElement.setText(Integer.toString(count));
			rs = stmt.executeQuery("select r.username, r.online_status, a.name, B.at, r.first_name, r.last_name from (select wu.wspid wuwspid, wu.uid wuuid, wu.gid wugid, max(h.accessedAt) at from workspace_user wu left join history h on wu.uid=h.uid and wu.wspid=h.wspid group by wu.wspid, wu.uid) B, registration r, access a where r.id=B.wuuid and B.wugid=a.id and B.wuwspid="+id);
			while (rs.next()) {
		...
		try {
			...
			int count = rs.next() ? rs.getInt(1) : 0;
			wspElement.setText(Integer.toString(count));
			rs = stmt.executeQuery("select r.username, a.createdAt, a.annotation, r.first_name, r.last_name from annotation a, registration r where a.creator = r.id and a.wspid = "+id+" order by r.username, a.createdAt");
			while (rs.next()) {
	...
	private OMElement createListWorkspaceElement(String userinfo) throws RemoteException {
		...
		try {
			...
			wspElement.setText(Integer.toString(count));
			rs = stmt.executeQuery("select n.name, r.username, r.first_name, r.last_name from network n, registration r, user_network un where n.owner=r.id and n.id=un.network_id and un.user_id="+uid+" order by n.name, r.username");
			while (rs.next()) {
		...
		try {
			...
			rs = stmt.executeQuery("select w.id, w.title, u.username, ga.name, w.locked, l.username, w.lastSync, w.createdAt, w.description, u.first_name, u.last_name, l.first_name, l.last_name from workspace w, registration u, registration l, workspace_user wu, access ga where w.creator = u.id and w.lastLockedUser = l.id and wu.wspid = w.id and wu.gid = ga.id and wu.uid = "+uid+ " order by w.id");
			while (rs.next()) {
	...
	private OMElement createLsLockWorkspaceElement(String userinfo) throws RemoteException {
		...
		try {
			rs = stmt.executeQuery("select count(*) from workspace where locked=TRUE and lastLockedUser="+uid);
			int count = rs.next() ? rs.getInt(1) : 0;
			wspElement.setText(Integer.toString(count));
			System.out.print("locked remote workspaces(" + count + "): ");
			rs = stmt.executeQuery("select id, title from workspace where locked=TRUE and lastLockedUser= "+uid);
			while (rs.next()) {
