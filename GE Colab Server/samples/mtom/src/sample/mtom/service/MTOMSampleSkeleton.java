...
public class MTOMSampleSkeleton {
	...
	private String getSvrTime(int id, String checkoutstr) throws Exception{
		...
		int lockuid = 0;
		String lockuname = "";
		String query = "select w.locked, w.lastLockedUser, r.username from workspace w, registration r where w.lastLockedUser = r.id and w.id="+id;
		...
		try {
		    ...
		    String msg = e.getMessage() + "\nFailed to retrieve access info from database for user " + uid;			    System.out.println(msg);
		    throw new RemoteException(msg);
		}
		...
		if (checkoutstr!=null && !checkoutstr.equals("") && !checkoutstr.equals("null")){
			...
			stmt.executeUpdate("update workspace set createdAt=createdAt, lastSync='"+now+"', locked=TRUE, lastLockedUser=" + uid +" where id="+id);
			stmt.executeUpdate("insert into history(wspid, uid, type, accessedAt) values ("
					   + id + ", " + uid + ", 'U', '" + now + "')");
		} else{
			Timestamp now = new Timestamp(new Date().getTime());
			String fpath = wspRoot + id + ".wsp";
			stmt.executeUpdate("update workspace set createdAt='"+now+"', lastSync='"+now+"', location='" + fpath + "' where id=" + id);
			stmt.executeUpdate("insert into history(wspid, uid, type, accessedAt) values ("
					   + id + ", " + uid + ", 'C', '" + now + "')");
		}
