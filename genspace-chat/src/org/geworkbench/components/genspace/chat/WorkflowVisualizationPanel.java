...
/**
 * This is used to display received workflows. It is a hack around the "WorkflowVisualization" class
 * @author jon
 *
 */
public class WorkflowVisualizationPanel extends JPanel {
	...
	public void draw(Node[] nodes, Edge[] edges)
	{
		...
		int height = this.getHeight() - 100; // subtract 100 to have a little buffer
...
//		Now actually do the layout, then recreate the display graph using the calculated co-ords.
		...
		layout.setSize(new Dimension(this.getWidth()-100,this.getHeight()-100));
		count = 0;
	...
	class GraphListener implements GraphModelListener, GraphSelectionListener
	{
		...
		public void valueChanged(GraphSelectionEvent e)
		{
			...
			if (o instanceof GraphNode)
			{
				...
				popup.show(WorkflowVisualizationPanel.this, (int)rect.getCenterX(), (int)rect.getCenterY());
			}
			/*
		...
		private boolean contains(String workflow, ArrayList<GraphNode> nodes)
		{
			...
			for (GraphNode node : nodes)
				if (workflow.contains(node.getUserObject().toString().replace("-<br>", "").replace("<html>","").replace("</html>","")) == false) return false;
			return true;
		}
	}
