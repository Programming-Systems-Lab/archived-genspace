...
/**
 * A specialized JPanel that receives hextile encoded PNG's and then
 * displays them
 * @author jon
 *
 */
public class ScreenSharingReceiver extends JPanel {
	...
	public void receiveImageUpdate(HashMap<String, Object> m, int w, int h)
	{
		...
		for(Object o : indices)
		{
			...
			try {
				im = ImageIO.read(new ByteArrayInputStream((byte[]) m.get("img" + tileNum)));
			} catch (IOException e) {
